...
def test_generate_puzzles_one_game() -> None:
    lichess_username = "Black"
    with AutoCloseStringIO(sample_one_game) as pgn:
        actual = list(generate_puzzles(pgn, lichess_username))
    ...
    assert len(expected_timestamps) == len(actual)
    assert len(expected_fens) == len(actual)
    assert len(expected_solutions) == len(actual)
